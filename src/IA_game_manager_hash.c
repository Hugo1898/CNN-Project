...
int choice_IA(int tab[2][6], int base, player *p_ordi, player *p_human, uint64_t hv, hash_type *hash_v, uint64_t hashSC[2][49], uint64_t hashP[2][6][49], uint64_t hashT[2][6][49]) {
	...
	best_move = -1;
	if (name == NORD) {
		ret = alpha_beta(- MAXV, MAXV, name, base, maxdepth, base, hv, hash_v, p_ordi, p_human, tab, &best_move, hashSC, hashP, hashT);
	}
	else {
		ret = alpha_beta(- MAXV, MAXV, name, base, maxdepth, base, hv, hash_v, p_human, p_ordi, tab, &best_move, hashSC, hashP, hashT);
	}
	time = clock()- otime;
